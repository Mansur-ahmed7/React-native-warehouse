...
export default function EditItemScreen() {
  ...
  // Photo state initialized with the existing alloy wheel image
  const [photoUri, setPhotoUri] = useState<any>(images.alloyWheel);
  const [showPhotoOptions, setShowPhotoOptions] = useState(false);

  // Controlled states pre-populated with existing item data matching screenshot exactly
  const [name, setName] = useState("Alloy Wheel 17 inch");
  const [partNumber, setPartNumber] = useState("AW-17-TY-2015");
  const [condition, setCondition] = useState<ItemCondition>("New");
  const [supplier, setSupplier] = useState("Ahmad Wheels Co.");
  const [buyPriceUSD, setBuyPriceUSD] = useState("120.00");
  const [sellPriceIQD, setSellPriceIQD] = useState("171,400");
  const [quantity, setQuantity] = useState("8");
  const [lowStockThreshold, setLowStockThreshold] = useState("3");

  // Compatible Cars list state pre-populated matching description exactly
  ...
  const [newBrand, setNewBrand] = useState<Brand>("Toyota");
  ...
  // Handle suggested sell price input format (adds commas dynamically)
    ...
    try {
      ...
      const { status } =
        await ImagePicker.requestMediaLibraryPermissionsAsync().catch((err) => {
          console.warn("[Photo] Permission request error:", err);
    ...
    try {
      ...
      const { status } =
        await ImagePicker.requestCameraPermissionsAsync().catch((err) => {
          console.warn("[Photo] Camera permission request error:", err);
          return { status: "granted" as const };
        });
  ...
  return (
    ...
    >
      ...
      >
        {/* Photo Area with existing photo and "Change Photo" Link */}
        <View className="items-center justify-center mb-6">
          <View className="w-[110px] h-[110px] rounded-2xl border border-gray-200 overflow-hidden bg-gray-50 shadow-sm relative mb-2">
            <Image
              source={
          ...
          >
            <Text className="font-poppins-bold text-[13px] text-[#0066FF]">
              Change Photo
            </Text>
          </Pressable>
        </View>
        ...
        <View className="gap-5 mb-8">
          ...
          <View>
            ...
            <TextInput
              ...
              placeholder="e.g. Ahmad Wheels Co."
              placeholderTextColor="#9CA3AF"
              className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 font-poppins-semibold text-[15px] text-text-primary shadow-sm"
            />
          </View>
          ...
          <View className="gap-2.5">
            <View className="flex-row gap-4">
              ...
              <View className="flex-1">
                ...
                <View className="w-full flex-row items-center bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                  <View className="bg-gray-50 border-r border-gray-200 px-3.5 py-3 items-center justify-center">
                  ...
                  <TextInput
                    ...
                    placeholder="120.00"
                    placeholderTextColor="#9CA3AF"
                    className="flex-1 font-poppins-semibold text-[15px] text-text-primary px-3 py-3"
                  />
                </View>
                {buyPriceUSD ? (
              ...
              <View style={{ flex: 1.25 }}>
                ...
                <View className="w-full flex-row items-center bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                  <TextInput
                    ...
                    placeholder="171,400"
                    placeholderTextColor="#9CA3AF"
                    className="flex-1 font-poppins-semibold text-[15px] text-text-primary px-3.5 py-3"
                  />
                  <View className="bg-gray-50 border-l border-gray-200 px-3 py-3 items-center justify-center">
            ...
            <Text className="font-poppins-medium text-[11px] text-gray-400 ml-1 mt-0.5">
              Staff can adjust this at time of sale
            </Text>
          </View>
          ...
          <View>
            ...
            <View className="w-full bg-white border border-gray-200 rounded-2xl p-4.5 shadow-sm">
              {/* Chip container */}
        ...
        <View className="gap-3.5">
          ...
          <Pressable
            onPress={handleDelete}
            className="w-full items-center justify-center rounded-[20px] bg-white border-2 border-red-500 py-4 active:bg-red-50/10"
          >
            <Text className="font-poppins-bold text-[16px] text-red-500">
              Delete Item
            </Text>
          </Pressable>
        </View>
